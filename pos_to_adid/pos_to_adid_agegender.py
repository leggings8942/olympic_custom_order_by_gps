...
display(df_count_agegender)

# COMMAND ----------

# 集計数5以上を抽出
df_count_age_inf = df_count_agegender\
    .join(
        df_infimum,on=['caption','place_id'],how='inner')\
    .filter(col('count')>=col('infimum'))\
    .drop('infimum')
display(df_count_age_inf)


# COMMAND ----------

# year,month,dtでpartitionを作成
df_count_agegender_output = df_count_age_inf\
...
display(df_count_agegender_output)
