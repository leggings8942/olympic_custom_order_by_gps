...
df_raw_data = spark.table(gps_table)\
      .filter(col('place').isin(place_list))\
      .filter(col('date')<=end)\
      .join(df_config,
            on=['place_id','competitor_id'],
            how='inner')\
      .filter(col('date')>=col('start_date'))\
      .dropDuplicates(['place_id','competitor_id','adid','date'])\
...
df_adid_count = df_raw_data\
    .groupBy('place_id','competitor_id','date')\
...
# adidとdateで突合して、重複データを削除する
df_duplication = df_raw_data\
    .join(df_comp_data,on=['adid','date'],how='inner')\
    .filter(
            (col('place_id')!= col('duplication_place_id')) | (col('competitor_id')!= col('duplication_competitor_id'))
        )\
    .drop_duplicates(['place_id','competitor_id','adid','date','duplication_place_id','duplication_competitor_id'])\
    .orderBy('place_id','competitor_id','date')
...
df_duplication_count = df_duplication\
    .groupBy('date','place_id','competitor_id','duplication_place_id','duplication_competitor_id')\
...
df_duplication_output = df_duplication_count\
    .join(df_adid_count,
          on=['place_id','date','competitor_id'],how='inner')\
    .orderBy(
        'place_id','competitor_id',
        'duplication_place_id','duplication_competitor_id','date')
...
if False:
    df_duplication_output\
        ...
        .partitionBy('place')\
        .csv(COMMON_PATH+f'output/duplication/daily/')

else:
    from typing import cast
    output_path = COMMON_PATH+f'output/duplication/daily/'
