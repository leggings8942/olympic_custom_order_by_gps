...
df_gps_raw_data = spark.table(gps_table)\
                    ...
                    .filter(col('date').isin(day_list))\
                    .withColumn('year', F.date_format(col('datetime'), 'yyyy'))\
                    .withColumn('month', F.date_format(col('datetime'), 'MM'))\
                    .withColumn('start_date', F.concat(col('year'),lit('-'),col('month'),lit('-01')))\
...
df_count = df_spot_with_place\
    .groupBy(['place_id','competitor_id','spot','prefecture','city','address', 'start_date'])\
    .agg(count('adid').alias('count'))\
    .orderBy(['place_id','competitor_id','spot','start_date'])
df_count.display()
...
df_count_spot_with_place_category = spark.table(spot_category_table)\
    ...
    .orderBy(['place_id','competitor_id','cohort_caption','spot','start_date'])
...
df_output = df_count_spot_with_place_category\
    ...
    .withColumn('year', F.date_format(col('start_date'), 'yyyy'))\
    .withColumn('month', F.date_format(col('start_date'), 'yyyy-MM'))
df_output.display()
